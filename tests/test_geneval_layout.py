...
class GenevalLayoutTest(unittest.TestCase):
    ...
    def test_geneval_shell_wrappers_have_valid_bash_syntax(self):
        result = subprocess.run(["bash", "-n", str(ROOT / "scripts/train/drpo-geneval.sh")], text=True, capture_output=True)
    ...
    def test_geneval_defaults_use_vit_l14_clip(self):
        ...
        wrapper_text = (ROOT / "scripts" / "train" / "drpo-geneval.sh").read_text(encoding="utf-8")
